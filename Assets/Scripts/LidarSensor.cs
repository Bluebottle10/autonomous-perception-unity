...
public class LidarSensor : MonoBehaviour
{
        ...
        bool current = true)
    {
        ...
        //reflectivity = (int)Mathf.Round(((intensityNreflectivity * 1000) - (int)(intensityNreflectivity * 1000)) * 100);
        id = (int)data.w;
    ...
    public void GetRangePointIntensity(int pointIndex,
    out float range,
    out Vector3 point,
    out float intensity,
    out int id,
    ...
    public bool GetRangesAndIntensities(out float[] ranges, out float[] intensities, bool current = true)
    {
        ...
        if (result)
        {
            for (int pointIndex = 0; pointIndex < TotalNumberOfData; pointIndex++)
            {
                ...
                GetRangeAndIntensity(idx, out ranges[pointIndex], out intensities[pointIndex], current);
            }
        }
        return result;
    }

    public bool GetRangesAndIntensitiesAndId(out float[] ranges, out float[] intensities, out int[] ids, bool current = true)
    {
        ranges = new float[TotalNumberOfData];
        intensities = new float[TotalNumberOfData];
        ...
        if (result)
        {
            ...
            {
                int idx = ScanIndex[pointIndex];
                GetRangeAndIntensityAndId(idx, out ranges[pointIndex], out intensities[pointIndex], out ids[pointIndex], current);
            }
        }
        return result;
    }
